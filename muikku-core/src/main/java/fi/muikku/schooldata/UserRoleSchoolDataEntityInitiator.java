...
@Stateless
@Dependent
@SchoolDataBridgeEntityInitiator ( entity = UserRole.class )
public class UserRoleSchoolDataEntityInitiator implements SchoolDataEntityInitiator<UserRole> {
	...
	@Override
	public UserRole single(UserRole userRole) {
	...
	@Override
	public List<UserRole> list(List<UserRole> userRoles) {
		List<UserRole> result = new ArrayList<>(userRoles.size());
		
		for (UserRole userRole : userRoles) {
			userRole = single(userRole);
			if (userRole != null) {
		  	result.add(userRole);
			}
		}
