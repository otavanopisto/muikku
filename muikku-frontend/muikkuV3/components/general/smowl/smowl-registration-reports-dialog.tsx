...
import { SmowlRegistrationReports } from "./smowl-registration-reports";
import { FetchError, ResponseError } from "~/generated/client";

/**
 * SmowlActivityResultsDialogProps
 */
interface SmowlRegistrationReportsDialogProps {
  activityId: number;
  lang: string;
 ...
 * @param props SmowlRegistrationReportsDialogProps
 */
const SmowlRegistrationReportsDialog = (
  props: SmowlRegistrationReportsDialogProps
) => {
  const { activityId, lang, dataLoader, children } = props;

  const [isOpen, setIsOpen] = React.useState(false);
  const { loading, smowlApiAccountInfo, aNamesJson } =
    useSmowlRegistrationReportsDialog({
  ...
  return (
    <Dialog
      disableScroll={true}
      title="SMOWL Registration Reports"
      content={content}
      modifier={["studies"]}
      onOpen={() => {
        setIsOpen(true);
 ...
 */
interface UseSmowlRegistrationReportsDialogProps {
  activityId: number;
  dialogOpen: boolean;
  dataLoader: (activityId: number) => Promise<{
    aNamesJson: string;
 ...
 */
const useSmowlRegistrationReportsDialog = (
  props: UseSmowlRegistrationReportsDialogProps
) => {
  ...
  React.useEffect(() => {
    ...
    const loadANamesJson = async () => {
      try {
        setaNamesJson((prev) => ({ ...prev, loading: true }));
...
export default SmowlRegistrationReportsDialog;
