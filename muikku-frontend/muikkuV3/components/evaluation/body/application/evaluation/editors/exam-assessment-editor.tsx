 ...
 */
interface ExamAssessmentEditorProps extends WithTranslation {
  // Redux props
  ...
  updateEvaluationExamEvaluationInfo: UpdateEvaluationExamEvaluationInfoTriggerType;
  /**
 ...
 */
interface ExamAssessmentEditorState {
 ...
 */
class ExamAssessmentEditor extends SessionStateComponent<
  ExamAssessmentEditorProps,
  ExamAssessmentEditorState
> {
   ...
   * @param props props
   */
  constructor(props: ExamAssessmentEditorProps) {
    super(props, `exam-assessment-editor`);

    const { materialEvaluation, selectedAssessment, exam } = props;

    const { evaluationInfo } = exam;
    // Draft id is used to save the state of the editor
    const draftId = `${selectedAssessment.userEntityId}-${exam.folderId}`;
   ...
   */
  componentDidMount = () => {
    const { materialEvaluation, exam } = this.props;
   ...
   * @param data.workspaceNodeId workspaceNodeId
   * @param data.dataToSave data ToSave
   * @param data.nodeId nodeId
  ...
  saveAssignmentEvaluationGradeToServer = async (data: {
    ...
    const { workspaceEntityId, userEntityId, workspaceNodeId, dataToSave } =
      data;
    ...
    try {
      const assessmentWithAudio = data.edit
        ? await evaluationApi.updateWorkspaceNodeAssessment({
            ...
            assessmentId: this.props.exam.evaluationInfo.id,
            updateWorkspaceNodeAssessmentRequest: {
      ...
      this.props.updateEvaluationExamEvaluationInfo({
        workspaceNodeId: workspaceNodeId,
        userEntityId: userEntityId,
      });
  ...
  handleSaveAssignment = async (
    ...
    await this.saveAssignmentEvaluationGradeToServer({
      ...
      userEntityId: userEntityId,
      workspaceNodeId: this.props.exam.folderId,
      dataToSave: {
        identifier: evaluationInfo ? evaluationInfo.id.toString() : undefined,
      ...
      nodeId: this.props.exam.folderId,
      defaultGrade,
      edit: !!evaluationInfo,
    });
  };
   ...
   */
  handleDeleteEditorDraft = () => {
    const { exam, materialEvaluation } = this.props;
    const { evaluationInfo } = exam;
    const { activeGradeSystems } = this.state;
...
export default withTranslation([
  ...
  "materials",
  "common",
])(connect(mapStateToProps, mapDispatchToProps)(ExamAssessmentEditor));
