...
class DeleteImageDialog extends React.Component<DeleteImageDialogProps, DeleteImageDialogState> {
  ...
  deleteImage(closeDialog: ()=>any){
    ...
    this.props.updateCurrentWorkspaceImagesB64({
      delete: true,
      success: ()=>{
        this.props.displayNotification(this.props.i18n.text.get("plugin.workspace.management.notification.coverImage.deleted"), "success");
        this.props.onDelete();
      },
    });
  }
  ...
  render(){
    let content = (closeDialog: ()=>any)=><div>
     {this.props.i18n.text.get('plugin.workspace.management.deleteImage.dialog.description')}
    </div>;
    let footer = (closeDialog: ()=>any)=>{
      return <div className="dialog__button-set">
        <Button buttonModifiers={["execute","standard-ok"]} onClick={this.deleteImage.bind(this, closeDialog)}>
          {this.props.i18n.text.get('plugin.workspace.management.deleteImage.dialog.deleteButton.label')}
        </Button>
        <Button buttonModifiers={["cancel","standard-cancel"]} onClick={closeDialog}>
          {this.props.i18n.text.get('plugin.workspace.management.deleteImage.dialog.cancelButton.label')}
        </Button>
      </div>
    }
    return <Dialog isOpen={this.props.isOpen} title={this.props.i18n.text.get('plugin.workspace.management.changeImage.dialog.title')}
      content={content} footer={footer} modifier="delete-header-image" onClose={this.props.onClose}/>
  }
}
