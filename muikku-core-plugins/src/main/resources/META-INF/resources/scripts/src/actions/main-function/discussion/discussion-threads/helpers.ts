...
export async function loadThreadsHelper(initial:boolean, areaId:number | null, dispatch:(arg:AnyActionType)=>any, getState:()=>any){
  ...
  try {
    let threads:DiscussionThreadListType = <DiscussionThreadListType>await promisify(actualAreaId ? mApi().forum.latest.read(actualAreaId, params) : mApi().forum.latest.read(params), 'callback')();
