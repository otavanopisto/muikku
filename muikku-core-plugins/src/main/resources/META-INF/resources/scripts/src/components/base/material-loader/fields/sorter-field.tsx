...
export default class SorterField extends React.Component<SorterFieldProps, SorterFieldState> {
  ...
  swap(itemA: SorterFieldItemType, itemB: SorterFieldItemType){
    ...
    //items are update with the swapped version, and after that's done we check answers
    this.setState({
      items
    }, this.checkAnswers);
  }
  checkAnswers(){
    ...
    //In this case whether it overall correct or not depends son whether
    ...
    return <Element className={`material-page__sorter-wrapper ${elementClassNameState}`}>
      <Element className="material-page__sorter-items">
       {this.state.items.map((item, index)=>{
         ...
         //Now we might be able if we are asked to to show the answer of the very specific item
         //this only happens if the answer is incorrect because otherwise is correct and it's unnecessary
         //we set them up so that they show each if they are correct or incorrect
         ...
         //the parent component is a class name always make sure to have the correct class name not to overflow
