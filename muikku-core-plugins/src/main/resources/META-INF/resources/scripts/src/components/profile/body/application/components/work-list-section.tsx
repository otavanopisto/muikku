...
import { StoredWorklistItem, WorklistBillingState, WorklistSection } from "~/reducers/main-function/profile";
...
export function WorkListSection(props: WorkListSectionProps) {
  ...
  const entries = props.isExpanded && hasData ? (
    sortBy(props.section.items, sortByProperty, sortByDirection as any).map((item) => {
      return (
        <WorkListRow key={item.id} item={item} currentMonthDayLimit={props.currentMonthDayLimit} />
      );
    })
  ) : null;

  // calculate the months total value of worklist entries
  let totalCostSummary: number = props.isExpanded && hasData ? (
    props.section.items.map(item => (item.price * item.factor)).reduce((a, b) => a + b)
  ) : null;

  const sectionTotalRow = (<div className="application-sub-panel__item application-sub-panel__item--worklist-total">
    <div className="application-sub-panel__item-title application-sub-panel__item-title--worklist-total">{props.i18n.text.get("plugin.profile.worklist.worklistEntriesTotalValueLabel")}</div>
  ...
  const sectionHasSubmittableEntries = props.section.items && props.section.items.some((i) => i.state === WorklistBillingState.ENTERED);

  // check if section is for previous month entries
  const sectionIsPreviousMonth = moment(props.section.summary.beginDate).isSame(props.previousMonthsFirstDay);

  // check if section is for current month entries
  const sectionIsCurrentMonth = moment(props.section.summary.beginDate).isSame(props.currentMonthsFirstDay);

  // check if current months date is 10th or less so user can still submit previous months etries
  const isPreviousMonthAvailable = props.daysInCurrentMonth <= props.currentMonthDayLimit;
  ...
  const submitLastMonthButton = (
    <SubmitWorklistItemsDialog summary={props.section.summary}>
      <Link className="link link--submit-worklist-approval">{props.i18n.text.get("plugin.profile.worklist.submitWorklistForApproval")}</Link>
  ...
  const shouldHaveSubmitEntriesLinkAvailable = props.isExpanded && sectionHasSubmittableEntries &&
    ((isPreviousMonthAvailable && sectionIsPreviousMonth) || sectionIsCurrentMonth);
  ...
  const sectionLabels = (<div className="application-sub-panel__multiple-items application-sub-panel__multiple-items--item-labels">
    <span className="application-sub-panel__multiple-item-container application-sub-panel__multiple-item-container--worklist-description">
      <label className="application-sub-panel__item-title application-sub-panel__item-title--worklist-list-mode">
        <Link
          ...
          onClick={onClickOnPropertyToSort.bind(null, "description")}>{props.i18n.text.get("plugin.profile.worklist.description.label")}
        </Link>
    ...
    <span className="application-sub-panel__multiple-item-container application-sub-panel__multiple-item-container--worklist-date">
      <label className="application-sub-panel__item-title application-sub-panel__item-title--worklist-list-mode">
        <Link
          className="link link--worklist-entries-sorting"
          onClick={onClickOnPropertyToSort.bind(null, "entryDate")}>{props.i18n.text.get("plugin.profile.worklist.date.label")}
        </Link>
  ...
  return (
    <div className="application-sub-panel application-sub-panel--worklist">
      <h4 onClick={props.onToggleSection} className="application-sub-panel__header application-sub-panel__header--worklist-entries">
        <ButtonPill buttonModifiers="expand-worklist" icon={props.isExpanded ? "arrow-down" : "arrow-right"} as="span" />
        <span>{props.section.summary.displayName} ({props.section.summary.count})</span>
      </h4>
