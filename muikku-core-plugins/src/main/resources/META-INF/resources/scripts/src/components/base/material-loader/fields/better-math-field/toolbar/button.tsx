...
export default class ToolbarButton extends React.Component<ToolbarButtonProps, ToolbarButtonState>{
  ...
  render(){
    return <button className={this.props.className} onMouseDown={this.onMouseDown} onMouseEnter={this.onMouseEnter}
      onMouseLeave={this.onMouseLeave} onTouchStart={this.onTouchStart} onTouchEnd={this.onTouchEnd}
      onContextMenu={this.disableMenu} style={{userSelect: "none"}}>
      {this.props.html ? <span dangerouslySetInnerHTML={{__html: this.props.html}}/> : null}
      {this.props.image ? <img src={this.props.image}/> : null}
      {this.state.tooltipVisible ? <span className={this.props.tooltipClassName}>{this.props.tooltip}</span> : null}
    </button>
  }
}
