...
class GuiderApplication extends React.Component<GuiderApplicationProps, GuiderApplicationState> {
  ...
  render() {
    ...
    let primaryOption = <div className="form-element form-element--main-action">
      <label htmlFor="selectUsers" className="visually-hidden">{this.props.i18n.text.get('plugin.coursepicker.select.label')}</label>
      <select id="selectUsers" className="form-element__select form-element__select--main-action" disabled>
        <option>{this.props.i18n.text.get("plugin.guider.students.all")}</option>
      </select>
    </div>
    return (<div className="application-panel-wrapper">
      <ApplicationPanel modifier="guider" primaryOption={primaryOption} toolbar={toolbar} title={title} asideBefore={this.props.aside}>
        <Students />
      </ApplicationPanel>
    </div>);
  }
