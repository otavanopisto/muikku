...
interface OrganizerFieldState {
  ...
  //Wheter it was correct or incorrect overall
...
export default class OrganizerField extends React.Component<OrganizerFieldProps, OrganizerFieldState> {
  ...
  checkAnswers(){
    ...
    //And we use the overall state to call the answer change function
  ...
  render(){
    //The overall state if we got one and we check for answers
    ...
    return <div className={`muikku-organizer-field muikku-field ${elementClassNameState}`}>
      ...
      <div className="muikku-categories-container flex-row">
        {this.props.content.categories.map((category)=>{
          //we make a category class name for if the answer state is there, only worth it if the whole thing is not correct
          //if the whole thing is correct then every category is correct
          ...
          //we first check whether the category is correct
          let weCheckAnswerAndCategoryIsCorrect = this.props.checkAnswers &&
          ...
          //if we are asked to display and the answers are not correct then we add the missing items
          if (this.props.displayCorrectAnswers && !weCheckAnswerAndCategoryIsCorrect){
          ...
          return <Droppable interactionGroup={this.props.content.name} onClick={this.selectBox.bind(this, category)}
            ...
            <div className="muikku-category">{this.state.boxes[category.id].map((termId)=>{
              //showhing whether terms are correct or not is only worth it is whole answers are not correct and the category itself is not correct
              //otherwise it's reduntant, if the whole thing is correct or the category is correct then every term is correct too
