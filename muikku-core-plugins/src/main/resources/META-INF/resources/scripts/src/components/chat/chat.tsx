  ...
  sendMessage(event: any){ 
    ...
    reactComponent.state.converse.api.rooms.open(jid).then((chat: any) => {
      ...
      let message = chat.messages.findWhere('correcting')
        ...
        //chat.addHandler('message', 'x', reactComponent.getMUCMessages(chat) );

        /* let groupMessage = chat.messages.models.map((model) => ({
            from: model.attributes.from,
            content: model.attributes.message
          }))
