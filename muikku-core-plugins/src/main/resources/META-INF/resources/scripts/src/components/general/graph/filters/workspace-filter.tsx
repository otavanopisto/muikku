  ...
  render() {
    ...
    this.props.workspaces.map((workspace) => {
      ...
      items.push(<div className={"filter-item filter-item--workspaces" + modificator} key={workspace.name}>
        <input id={`filterWorkspace` + workspace.id} type='checkbox' onClick={() => { this.props.workspaceHandler(workspace.id) }} checked={ifChecked} />
        <label htmlFor={`filterWorkspace` + workspace.id} className="filter-item__label">{workspace.name}</label>
      </div>);
    });
    ...
    if (this.props.completedWorkspaces && this.props.completedWorkspaces.length > 0 && this.props.filteredCompletedWorkspaces) {
      ...
      this.props.completedWorkspaces.map((workspace) => {
        ...
        let modificator = workspace.isEmpty ? "-empty" : "";
        items.push(<div className={"filter-item filter-item--workspaces" + modificator} key={workspace.name}>
          <input id={`filterWorkspace` + workspace.id} type='checkbox' onClick={() => { this.props.completedWorkspaceHandler(workspace.id) }} checked={ifChecked} />
          <label htmlFor={`filterWorkspace` + workspace.id} className="filter-item__label">{workspace.name}</label>
        </div>)
      });
    }
