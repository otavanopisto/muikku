...
export default class ConnectField extends React.Component<ConnectFieldProps, ConnectFieldState> {
  ...
  triggerChange(){
    //whenever we get a change, check answers
    this.checkAnswers();
  ...
  checkAnswers(){
    //if we are not allowed to check answer then return
    ...
    return <div className={`muikku-connect-field muikku-field ${elementClassNameState}`}>
      <div className="muikku-connect-field-terms">
        {this.state.fields.map((field, index)=>{
          //the item class name only necessary if it was a fail and we are checking answers
      ...
      <div className="muikku-connect-field-counterparts">
       {this.state.counterparts.map((field, index)=>{
         ...
         let itemCorectAnswerComponent = null;
         //we need to do this
         if (this.props.displayCorrectAnswers && !(this.props.checkAnswers && itemAnswer === "PASS")){
           //this is just a component giving an overview, of which number was meant to be the correct answer
           itemCorectAnswerComponent = <span className="muikku-connect-field-number">
         ...
         return <Draggable interactionData={{field, index, isCounterpart: true}} 
           ...
           className={className} key={field.name} style={style}>{itemCorectAnswerComponent}{field.text}</Draggable>
       })}
      </div>
    </div>
  }
