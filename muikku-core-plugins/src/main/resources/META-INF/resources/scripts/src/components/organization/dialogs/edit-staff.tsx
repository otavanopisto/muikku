...
import {updateStaffmember, ApplyStaffmemberTriggerType} from '~/actions/main-function/users';
...
interface OrganizationUserProps {
  ...
  updateStaffmember: ApplyStaffmemberTriggerType
}
...
class OrganizationUser extends React.Component<OrganizationUserProps, OrganizationUserState> {
  ...
  cancelDialog (closeDialog: ()=> any) {
    this.setState({
      user: {role: this.props.data.role, studyProgrammeIdentifier: ""},
      firstNameValid: 1,
      lastNameValid: 1,
      emailValid: 1,
    });
  ...
  render(){
      ...
      <div>
        <DialogRow modifiers="new-user">
          <SelectFormElement name="role" modifiers="new-user" label={this.props.i18n.text.get('plugin.organization.users.addUser.label.role')} updateField={this.updateField}>
            ...
            <option value="STUDY_PROGRAMME_LEADER" {...this.props.data.role && this.props.data.role == "STUDY_PROGRAMME_LEADER" ? "selected" : null}>{this.props.i18n.text.get('plugin.organization.users.addUser.role.studyProgrammeLeader')}</option>
            <option value="STUDY_GUIDER" {...this.props.data.role && this.props.data.role == "STUDY_GUIDER" ? "selected" : null}>{this.props.i18n.text.get('plugin.organization.users.addUser.role.studyGuider')}</option>
          </SelectFormElement>
        </DialogRow>
        <DialogRow modifiers="new-user">
    ...
    return(
      <Dialog modifier="new-user" title={this.props.i18n.text.get('plugin.organization.users.addUser.title')} content={content} footer={footer}>
        {this.props.children}
      </Dialog>
    )
  }
