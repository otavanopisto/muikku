...
class Journal extends React.Component<JournalProps, JournalState>{
  render() {
    let student = this.props.workspace.students && this.props.workspace.students.results.find(s => s.userEntityId === this.props.journal.userEntityId);
    return <ApplicationListItem className="journal">
      ...
      <ApplicationListItemBody className="application-list__item-body">
        {!this.props.status.isStudent ?
          student ? <header className="application-list__item-content-header application-list__item-content-header--journal-entry">{this.props.journal.title}</header> : null
          : null}
