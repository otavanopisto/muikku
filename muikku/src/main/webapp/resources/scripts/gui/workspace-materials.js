(function() {
  ...
  $(document).ready(function() {
    ...
    function loadMaterial(sel, $node) {
      var materialType = $node.data('material-type');
      switch (materialType) {
      case 'html':
        var materialId = $node.data('material-id');
        mApi().materials.html.read(materialId).callback(function(err, html) {
          $node.html(html.html);
          var next = $node.next(sel);
          if (next.length) {
            loadMaterial(sel, next);
          }
        });
        break;
      case 'folder':
    ...
    loadMaterial('.workspace-materials-view-page', $('.workspace-materials-view-page').first());
